...
P.add_argument('--section-refine', default='')
...
def face_y(x,z):
    ...
    return -1.20-(1.00+cheek+muzzle)*q**.33
        ...
        for back in [False,True]:
            for k in range(rings+1):
                ...
                for j in range(1 if k==0 else n):
                    ...
                    y+= .105*math.sqrt(max(0,1-r*r))+.020 if back else -.040+.105*(1-r*r)-.160*math.exp(-((up-.40)/.42)**2)*(1-r*r)
...
def main():
    ...
    for ob in COL.objects:
        ...
        for v in ob.data.vertices:
            ...
            # Distinct compact forehead mantle before a broader dorsal torso.
            # Its lateral limit is carried by the shoulders/ears, so compacting
            # the anterior upper interior does not simply shrink the whole body.
            mantle=math.exp(-((y+1.28)/.44)**2)*upper
            dorsal=math.exp(-((y+.35)/.60)**2)*upper
            v.co.x*=1-.085*mantle+.030*dorsal
            v.co.z+=-.020*mantle+.025*dorsal
            if ob.name!='FL_TAIL':
                ...
                v.co.y+=.065*math.exp(-((y-1.80)/.25)**2)*(1-math.exp(-(x/.45)**4))
    if ARGS.section_refine:
        assert ARGS.profile_fit, 'Section refinement follows the saved base calibration'
        guide=json.loads((ROOT/ARGS.section_refine).read_text())
        xs=np.array(guide['top_x_scale_by_world_y']);zs=np.array(guide['side_crown_z_offset_by_world_y'])
        for ob in COL.objects:
            if ob.type=='MESH':
                for vertex in ob.data.vertices:vertex.co=profile_deform(vertex.co)
        for key,p in list(LANDMARKS.items()):LANDMARKS[key]=list(profile_deform(p))
        # Recover the broad dorsal shoulders visible in Front without widening
        # the face mantle or moving the independently placed ears and feet.
        for ob in COL.objects:
            if ob.type=='MESH' and (ob.name.startswith('FL_') or ob.name.startswith('Fleece_')):
                for vertex in ob.data.vertices:
                    x,y,z=vertex.co
                    vertex.co.x*=1+.060*math.exp(-((y+.65)/.65)**2)*max(0,min(1,(z-1.45)/.60))
    ...
    cross_sections={str(z):[[float(x),face_y(float(x),z),z] for x in np.linspace(-.85,.85,41) if abs(x/cheek_width(z))**2.0+abs((z-1.15)/.57)**2.5<1] for z in [.65,.85,1.00,1.145,1.45,1.65]}
    (folder/'geometry-data.json').write_text(json.dumps({'pass':ARGS.pass_number,'iteration':ARGS.iteration,'landmarks_3d':LANDMARKS,'cameras':camera_data,'clusters':CLUSTERS,'sections':SECTIONS.tolist(),'face_cross_sections':cross_sections,'resolution':ARGS.resolution,'profile_fit':ARGS.profile_fit,'section_refine':ARGS.section_refine,'camera_fit':ARGS.camera_fit,'deformation':'iteration>=5: x waist gaussian 20%, shoulder gaussian +7.5%; y*0.967; shared across all views; sections/clusters remain authoring-space'},indent=2),encoding='utf8')
