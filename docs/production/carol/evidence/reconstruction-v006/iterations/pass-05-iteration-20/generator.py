...
def face_y(x,z):
    ...
    muzzle=.075*math.exp(-((x/.28)**2+((z-1.08)/.16)**2))
    return -1.20-(1.025+cheek+muzzle)*q**.20
...
def ears():
    for sign,label in [(-1,'L'),(1,'R')]:
        ...
        for back in [False,True]:
            for k in range(rings+1):
                ...
                for j in range(1 if k==0 else n):
                    ...
                    y=-1.72+.84*t-.075*math.sin(math.pi*t)
                    y+= .105*math.sqrt(max(0,1-r*r))+.020 if back else -.040+.105*(1-r*r)
        ...
        ellipsoid('Ear_root_'+label,(sign*.86,-1.65,1.43),(.12,.16,.10),BROWN)
...
def hooves():
    for y,region in [(-1.20,'front'),(1.24,'rear')]:
        for sign,label in [(-1,'L'),(1,'R')]:
            ...
            levels=[(.008,.72),(.016,.87),(.05,.97),(.12,1),(.27,.98),(.39,.89),(.46,.72)]
            for z,radius in levels:
                for i in range(n):
                    u=math.tau*i/n
                    x=.265*radius*math.copysign(abs(math.cos(u))**.82,math.cos(u))
                    yy=.318*radius*math.copysign(abs(math.sin(u))**.82,math.sin(u))
...
def cloud(name,c,r,normal,seed,material=WHITE):
    ...
    def fn(u,v):
        ...
        rho=(1+.035*math.sin(3*u+.7*seed)*math.sin(v)**2) if secondary else .975*sum(t**14 for t in radii)**(1/14)
        q=Vector(c)+e*(r[0]*sx*rho)+f*(r[1]*sy*rho)+n*(r[2]*sz*rho)
        return tuple(q)
    ob=param_surface(name,fn,64,40,material);ob['cluster_id']=name;ob['scale_hierarchy']='chassis / major scalloped groups / quiet recess domes'
...
def fleece():
    ...
    cloud('FL_TAIL',(.02,2.04,1.70),(.35,.37,.30),(0,1,.3),512)
