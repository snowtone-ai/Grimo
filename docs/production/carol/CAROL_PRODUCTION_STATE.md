...
- Exposed-Hero module reference decision (2026-09-24): the Human-reviewed Carol **ear** and **hoof** four-view module sheets are accepted as localized production helper authorities and are stored at `assets/grimo/source/carol/approved-3d/modules/carol-ear-module-authority.webp` and `assets/grimo/source/carol/approved-3d/modules/carol-hoof-module-authority.webp`. They remain subordinate to canonical identity and the four locked Normal/Skin references; the 3/4 panels are validation-only and the cream attachment stubs are orientation helpers, not final visible geometry.
- Ear/hoof implementation rule: preserve the retained high-quality v011 front face; replace the temporary predecessor ear/hoof modules as bounded modules using the approved module sheets plus the numerical locks in `CAROL_GEOMETRY_PARAMETERS.md`. Do not rebuild the whole character merely to replace these modules.
- Fleece motion decision (2026-09-24): Carol's fleece is a **body-attached stylized cloud mass**, not loose fur. Macro fleece position follows its owning head/torso region with effectively zero perceptible transform lag. Do not use sliding shell follow-through, independent cloud-ball jiggle, or physics-led broad motion. Softness is expressed through low-amplitude local compression/opening/recovery and, only when useful, a very small authored broad **shape** settle. Mesh separation may remain for production control; it does not imply independent motion ownership.
...
- No whole-character geometry, fleece, rig, animation, GLB, runtime, canonical identity, or locked Normal/Skin reference was promoted by the earlier doctrine update. The 2026-09-24 decision adds only the two localized ear/hoof module helper authorities described above.
- **Next handoff:** implement the bounded ear/hoof Hero-module upgrade while preserving the v011 face, then adapt/reuse the strongest compatible full-spatial fleece donor under the body-attached fleece rule. Motion remains downstream of a Human-evaluable Full-Spatial Carol Neutral.
