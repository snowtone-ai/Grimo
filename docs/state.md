...
Carol production infrastructure and the authoritative source packet are staged under `docs/production/carol/`, `docs/grimo/knowledge/`, and `assets/grimo/source/carol/approved-3d/`; no modeling has started. The existing `assets/grimo/source/carol/carol-Identity-canonical.png` remains the visible-identity authority, the approved 3D packet is recorded by `assets/grimo/source/carol/approved-3d/authority.json`, and `CAROL_MVP_MOTION_SPEC.md` is the Carol Motion implementation authority. The next geometry interpretation is ready to begin, while `CAROL_GEOMETRY_DECISION.md` remains intentionally undecided.
