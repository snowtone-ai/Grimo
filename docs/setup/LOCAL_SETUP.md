...
6. Optionally run `pnpm check:fast` for a lightweight type check.
7. Run `pnpm dev` and open `http://localhost:3000`. `pnpm check:full` (or compatibility alias `pnpm verify`) is for PR, release, or troubleshooting validation.
