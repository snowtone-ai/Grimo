...
Status: GEOMETRY FAIL; iteration 16 is the latest clean restore after camera
correspondence and shared rear/ear-volume candidates were rejected. Human Gate PENDING.
...
`assets/grimo/production/carol/blender/carol-a-v006.blend` (pass 5, iteration 16 clean restore).
...
Do not hide failures with materials or view-dependent shape. Camera-only 3Q
candidates raised isolated IoU only by violating face/ear/hoof correspondence
(up to 19% normalized residual); do not retain those camera changes as a pass.
