...
export function sortTasksByTime<T extends TaskDateFields>(tasks: T[]): T[] { return [...tasks].sort(compareDueTime); }
export function sortTasksByDateTime<T extends TaskDateFields>(tasks: T[]): T[] {
  return [...tasks].sort((a,b) => a.dueDate !== b.dueDate ? a.dueDate.localeCompare(b.dueDate) : compareDueTime(a,b));
}

export interface CalendarDaySummary { completed: number; pending: number }
export function buildCalendarSummary(tasks: TaskDateFields[], year: number, month: number): Record<string, CalendarDaySummary> {
  ...
  for (let day=1; day<=daysInMonth; day++) {
    ...
    for (const task of tasks) {
      if (task.completedAt?.slice(0,10) === dateStr) completed++;
      if (doesTaskApplyToDate(task,dateStr) && !taskForDisplayDate(task,dateStr).completed) pending++;
    }
    if (completed || pending) summary[dateStr] = {completed,pending};
  }
  return summary;
}
