...
SOURCE = ROOT / 'assets/grimo/production/carol/blender/carol-ear-production-v001.blend'
OUTPUT = SOURCE.with_name('carol-ear-production-v002.blend')
...
HEAD = 'f2a8d40807099ce8c99bac2bf3afda38a03556f0'
EARS = {'EAR_L', 'EAR_R'}
...
STATIONS = [
    ...
    (0.014, .052, -.047, .032, .031, .000, -.022),
    (0.040, .052, -.085, .038, .033, .001, -.032),
    (0.085, .033, -.140, .047, .038, .006, -.061),
    (0.145, .004, -.183, .065, .049, .015, -.102),
    (0.210, -.028, -.217, .080, .057, .024, -.117),
    (0.270, -.057, -.224, .084, .055, .027, -.114),
    (0.315, -.073, -.211, .073, .047, .023, -.120),
    (0.345, -.087, -.194, .061, .039, .016, -.166),
    (0.365, -.102, -.180, .045, .030, .008, -.162),
    (0.380, -.119, -.166, .029, .020, .002, -.155),
    (0.387, -.134, -.155, .014, .011, .000, -.149),
]
...
def local_cage():
    ...
    for u, top, bottom, front, back, offset, pink in STATIONS:
        ...
        height = top-bottom
        rim = min(.026, height*.23)
        lower_pink=bottom+rim
        pink = max(lower_pink+.0008, min(top-rim, pink))
        basin = min(.013, front*.20, height*.09,(pink-lower_pink)*.28)
    ...
    for i in range(len(STATIONS)-1):
        for j in range(NC):
            ...
            mats.append(1 if 7 <= j <= 10 and 1 <= i < 8 else 0)
...
def snapshot(ob):
    ...
    if ob.type=='CURVE':
        h.update(struct.pack('<2f',ob.data.bevel_depth,ob.data.extrude))
        for spline in ob.data.splines:
            for point in spline.bezier_points:
                for co in [point.co,point.handle_left,point.handle_right]:h.update(struct.pack('<3f',*co))
            for point in spline.points:h.update(struct.pack('<4f',*point.co))
...
def material(name,color):
    ...
    if 'cocoa' in name:
        ...
        bump=m.node_tree.nodes.new('ShaderNodeBump');bump.inputs['Strength'].default_value=.16;bump.inputs['Distance'].default_value=.0006
...
def main():
    ...
    brown=material('CAROL ear production cocoa',(.15,.046,.021))
    ...
    validation={'source_branch':'origin/codex/carol-ear-production-v001','source_head':HEAD,
                ...
                'front_root_to_tip_station_angle_degrees':math.degrees(math.atan2(-(STATIONS[-1][1]+STATIONS[-1][2])*.5-.040,.387*math.cos(SWEEP))),
                'head_edits':0,
                'donor_audit':{'candidates':{n:{'present':v['present'],'sha256':v.get('sha256')} for n,v in json.loads((TMP/'audit.json').read_text()).items()},'chosen':'carol-ear-production-v001.blend','reason':'Best rounded forehead / under-jaw and seated-eye balance; face identical to hero-modules-v003. No inspected older donor improved the combined side-face read.'},
                'ear_surface':{'roughness':.86,'specular_ior_level':.18,'sheen_weight':.12,'brown_tip_from_u_H':.345},
