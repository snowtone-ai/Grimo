...
def face_y(x,z):
    # Rounded superelliptic forehead/cheek/jaw, .92 full AP depth.
    q=abs(x/.86)**2.0 + abs((z-1.15)/.57)**2.5
    return -1.20-1.05*max(.002,1-q)**.30-.055*math.exp(-((x/.38)**2+((z-1.03)/.24)**2))

def face():
    def fn(u,v):
        x=.86*math.sin(v)*math.cos(u)
        z=1.15+.57*math.copysign(abs(math.cos(v))**.8,math.cos(v))
        q=max(0,1-abs(x/.86)**2.0-abs((z-1.15)/.57)**2.5)
        front=math.sin(u)<0
        y=-1.20+(-1.05*q**.30 if front else .28*math.sqrt(q))
        if front:y-=.055*math.exp(-((x/.38)**2+((z-1.03)/.24)**2))*math.sqrt(q)
        return x,y,z
    ob=param_surface('Head_Volumetric_Cheeks_Jaw',fn,128,80,CREAM)
    ob['full_depth']='1.33 world units; forehead, cheek, muzzle and jaw sections exported'
    marker('face_left',(-.86,-1.20,1.15)); marker('face_right',(.86,-1.20,1.15))
    ...
    for sign,label in [(-1,'L'),(1,'R')]:
        ...
        for back in [False,True]:
            for k in range(rings+1):
                ...
                for j in range(1 if k==0 else n):
                    ...
                    x=1.22+.43*along;z=1.22-.12*along+.185*up*(1+.17*along)
                    y=-1.35+.55*along-.18*math.exp(-((along+.2)/.55)**2)
                    y+= .13*math.sqrt(max(0,1-r*r)) if back else -.065+.09*(1-r*r)
        ...
        for f in ob.data.polygons:
            if all(i<count for i in f.vertices):
                ...
                xx=(abs(center.x)-1.22)/.43;zz=(center.z-1.22+.12*xx)/.185
                if xx*xx+zz*zz < .75 and zz<.35:f.material_index=1
        marker('ear_root_'+label,(sign*.82,-1.90,1.34));marker('ear_tip_'+label,(sign*1.65,-.80,1.10))
...
def hooves():
    for y,region in [(-1.20,'front'),(1.24,'rear')]:
        for sign,label in [(-1,'L'),(1,'R')]:
            c=(sign*.53-.065,y,.235)
            def fn(u,v):
                x=.273*math.sin(v)*math.cos(u);yy=.335*math.sin(v)*math.sin(u);z=.31*math.cos(v)
                z=max(z,-.228)
                # Recess between two toes, concentrated in lower front contour.
                front=max(0,-math.sin(u))**5;low=max(0,-math.cos(v))**2
                yy+=.058*math.exp(-(x/.046)**2)*front*low
                z+=.052*math.exp(-(x/.040)**2)*front*low
                return c[0]+x,c[1]+yy,c[2]+z
            ob=param_surface('Hoof_'+region+'_'+label,fn,96,64,BROWN)
...
def cloud(name,c,r,normal,seed,material=WHITE):
    ...
    rng=np.random.default_rng(seed);count=5+seed%3
    angles=[math.tau*i/count+float(rng.uniform(-.2,.2)) for i in range(count)]
    lobes=[(np.array([.06,-.03,.22]),.73)]
    for a in angles:
        reach=float(rng.uniform(.43,.56));rad=float(rng.uniform(.44,.58))
        lobes.append((np.array([reach*math.cos(a),reach*math.sin(a),float(rng.uniform(-.04,.14))]),rad))
    def fn(u,v):
        sx=math.sin(v)*math.cos(u);sy=math.sin(v)*math.sin(u);sz=math.cos(v)
        direction=np.array([sx,sy,sz]);radii=[]
        for center,rad in lobes:
            dot=float(direction@center);disc=dot*dot+rad*rad-float(center@center)
            if disc>=0:radii.append(max(.01,dot+math.sqrt(disc)))
        # Smooth radial union of unequal authored lobes, one manifold cluster.
        rho=sum(t**32 for t in radii)**(1/32)
        q=Vector(c)+e*(r[0]*sx*rho)+f*(r[1]*sy*rho)+n*(r[2]*sz*rho)
        return tuple(q)
    ob=param_surface(name,fn,64,40,material);ob['cluster_id']=name;ob['scale_hierarchy']='chassis / major cloud / 5-7 unequal integrated lobes'
    CLUSTERS.append({'id':name,'center':c,'radii':r,'normal':normal,'lobe_count':count,'seed':seed})
    return ob
...
def fleece():
    ...
    cloud('FL_TAIL',(.02,1.94,1.61),(.33,.36,.34),(0,1,.3),512)
    if ARGS.iteration >= 3:
